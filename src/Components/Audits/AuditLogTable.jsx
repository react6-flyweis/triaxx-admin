...
const AuditLogTable = () => {
  ...
  const { data: auditsData, isLoading: loading, error } = useAudits();
  ...
  return (
    <div className="w-full overflow-y-auto poppins-text px-4 sm:px-6 lg:px-8">
      ...
      {/* Table (always render header; show skeleton or error inside) */}
      <div className="overflow-x-auto w-full">
        <div className="min-w-3xl">
          ...
          {/* Content: loading skeleton, error message, or rows */}
          {loading ? (
            ...
            <div className="w-full py-6 px-4">
              <div className="text-center py-6 text-yellow-700 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p className="mb-2">
                  ⚠️ {error?.response?.data?.message || String(error)}
                </p>
              </div>
            </div>
          ) : // Normal rows or no-data message
