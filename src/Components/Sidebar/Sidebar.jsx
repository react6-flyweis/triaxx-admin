...
export default function Sidebar() {
  ...
  const displayRole = user?.Role_id?.role_name || "Admin";
  const avatarSrc = user?.avatar || user?.profile_image || profileAvatar;
  ...
  return (
    <>
      <div className="fixed left-0 top-0 w-[250px] sm:w-[280px] md:w-[250px] h-screen bg-linear-to-b from-[rgba(106,27,154,0.1)] to-[rgba(211,47,47,0.1)] flex flex-col z-40 overflow-hidden">
        ...
        <div className="flex flex-col h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent">
          {/* Logo */}
          <div className="flex items-center justify-center gap-[8.83px] mt-6 sm:mt-8 md:mt-10 mb-8 sm:mb-10 shrink-0">
            <img
              src={logo}
              className="w-[100px] sm:w-[120px] h-4 sm:h-5"
              alt="Logo"
            />
          </div>

          {/* Profile */}
          <div className="flex justify-center mb-8 sm:mb-10 px-4 shrink-0">
            <div className="flex items-center gap-3 sm:gap-[15.24px] px-3 sm:px-4 py-1.5 sm:py-[7.62px] bg-linear-to-b from-[rgba(106,27,154,0.1)] to-[rgba(211,47,47,0.1)] rounded-4xl w-full max-w-[148.57px] h-[50px] sm:h-[57.24px]">
              <div className="w-8 sm:w-10 h-8 sm:h-10 rounded-full flex items-center justify-center shrink-0">
                <img
                  src={avatarSrc}
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <p className="text-black font-medium text-sm sm:text-base leading-5 sm:leading-6 font-['Manrope'] truncate">
                  {displayName}
                </p>
                <p className="text-black font-light text-xs leading-4 sm:leading-[18px] font-['Manrope']">
                  {displayRole}
                </p>
              </div>
            </div>
            ...
            {menuItems.map(({ icon, label, route }, idx) => {
              ...
              return (
                <div
                  ...
                  className={`cursor-pointer flex items-center gap-2 sm:gap-2.5 px-4 sm:px-[22px] py-2 sm:py-3 rounded-[20px] sm:rounded-3xl transition-all duration-300 min-h-11 sm:min-h-12 ${
                    isSelected
                      ? "bg-linear-to-r from-[#6A1B9A] to-[#D32F2F] shadow-lg scale-[1.02] sm:scale-105"
                ...
                >
                  <img
                    src={icon}
                    alt={label}
                    className={`w-5 sm:w-6 h-5 sm:h-6 transition-all duration-300 shrink-0 ${
