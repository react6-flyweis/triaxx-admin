...
const TicketListItem = ({
  ...
  const statusButton = (
    <div className="relative">
      ...
      >
        ...
        />
        <span className="font-medium text-sm sm:text-lg text-white">
          {status}
        </span>
        {status !== "New" &&
      ...
      {(status === "On-Going" || status === "Resolved") &&
        isStatusDropdownOpen && (
  ...
  return (
    <div
      className={`flex flex-col items-start p-4 sm:p-6 gap-3 sm:gap-4 w-full bg-gradient-to-b from-purple-100 to-red-100 rounded-xl sm:rounded-2xl ${
        isFirst ? "" : "mt-6 sm:mt-10"
      }`}
      onClick={() => navigate(`/ticket-details/${ticketId}`)}
    >
      {/* Header Section - Responsive layout */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-3 sm:gap-0">
        <div className="flex items-center gap-3 sm:gap-4">
          <div
            className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full ${currentStatus.dot}`}
          ></div>
        ...
        <div className="flex flex-col sm:flex-row justify-start sm:justify-center items-start sm:items-end gap-2 w-full sm:w-auto">
          <p className="text-black font-medium text-xs sm:text-sm order-2 sm:order-1">
            Posted at 12:45 AM
          </p>
          <div className="order-1 sm:order-2">{statusButton}</div>
        </div>
      </div>

      {/* Title */}
      <h3 className="text-black font-semibold text-sm sm:text-base w-full leading-tight">
        {title}
      </h3>

      {/* Description */}
      <p className="text-black font-normal text-sm sm:text-base leading-relaxed w-full">
        {description}
      </p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-2 sm:gap-0">
        <div className="flex items-center gap-2">
          <div
            className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-cover bg-center"
            style={{ backgroundImage: "url(https://via.placeholder.com/28)" }}
          ></div>
          <p className="text-black font-medium text-xs sm:text-sm">
            {userName}
          </p>
        </div>
        <p className="text-transparent bg-clip-text bg-gradient-to-r from-purple-700 to-red-700 font-medium text-xs sm:text-sm">
          Open Ticket
        </p>
      </div>
    </div>
