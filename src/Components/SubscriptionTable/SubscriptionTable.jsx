...
const TableRow = ({
  ...
  <>
    ...
    <div className="hidden lg:flex items-center w-full bg-white border-b border-gray-200 px-4 xl:px-6 py-4">
      <div className="flex-1 text-black font-poppins text-sm xl:text-base">
        {id}
      </div>
      <div className="flex-[2] text-black font-poppins text-sm xl:text-base">
        {businessName}
      </div>
      <div className="flex-[2] text-black font-poppins text-sm xl:text-base">
        {planPurchased}
      ...
      <div className="flex-[3] text-black font-poppins text-sm xl:text-base">
        {transactionId}
      </div>
      <div className="flex-[2] flex justify-end">
        <button
          onClick={() => navigate(`/client-details/${client_id}`)}
          className="w-full max-w-[120px] xl:max-w-[129px] h-[35px] xl:h-[38px] bg-linear-to-b from-[#6A1B9A] to-[#D32F2F] rounded-lg text-white font-poppins text-xs xl:text-sm font-medium"
        >
          View more
        </button>
      </div>
    ...
    <div className="lg:hidden bg-white border border-gray-200 rounded-lg p-4 mb-4 shadow-sm">
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-black font-poppins text-base font-medium">
            {id}
          </span>
        </div>
        <button
      ...
      <div className="space-y-2">
        <div className="flex flex-col sm:flex-row sm:justify-between">
          ...
          <span className="text-black font-poppins text-sm font-medium">
            {businessName}
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between">
          ...
          <span className="text-black font-poppins text-sm break-all">
            {transactionId}
          </span>
        </div>
      </div>
    ...
    <div className="p-3 sm:p-4 lg:p-6">
      ...
      <div className="hidden lg:block w-full overflow-x-auto rounded-2xl border border-gray-400">
        <div className="min-w-[1024px]">
          ...
          {/* Table Rows */}
          {isLoading ? (
            <div className="p-6">Loading subscriptions...</div>
          ) : isError ? (
            <div className="p-6 text-red-600">
              Error: {error?.message || "Failed to load"}
            </div>
          ) : rows.length === 0 ? (
            <div className="p-6">No subscriptions found.</div>
          ) : (
            rows.map((item, index) => {
              const id = item?.Plan_map_Client_id ?? item?._id ?? index + 1;
              const businessName =
                item?.client_id?.Business_Name ||
                item?.client_id?.BusinessName ||
                "N/A";
              const planPurchased = `${item?.plan_id?.name || "Unknown"}${
                item?.plan_id?.plan_duration
                  ? ` (${item.plan_id.plan_duration})`
                  : ""
              }`;
              const date = item?.CreateAt
                ...
                : "—";
              const transactionId =
                item?.Transaction_id || item?.PaymentStatus || "—";
