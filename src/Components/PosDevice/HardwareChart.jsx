...
const CustomBar = (props) => {
  const { x, y, width, height, fill, chartHeight } = props;
...
const HardwareChart = ({
  ...
  // Transform API monthly data ("month", "counts") to the format used by the chart
  const data =
    monthlyData && monthlyData.length
      ? monthlyData.map((m) => ({
          name: m.month.split(" ")[0].slice(0, 3),
          sales: Number(m.counts || 0),
        }))
      : [
          { name: "Jan", sales: 0 },
          { name: "Feb", sales: 0 },
      ...
      <div className="bg-white rounded-3xl overflow-hidden shadow-lg p-6">
        ...
        <div className="mb-6">
          <h1 className="text-4xl font-bold text-[#344BFD] mb-2">
            {loading ? "Loading..." : `${totalPrice} XOF`}
          </h1>
          <div className="flex items-center gap-2">
            <svg
            ...
            <span className="text-[#34C759] text-lg">
              {loading ? "" : `${sold} Sold`}{" "}
              <span className="text-[#00000099]">than Last Month</span>
            </span>
          </div>
        </div>
